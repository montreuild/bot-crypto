...
| ~~Haute~~ ✅ fait | **Gate Deflated Sharpe au naissance** | `opt_scoring.deflated_sharpe_ratio` (Bailey & López de Prado 2014) dans `beats_baseline()`. `app/core/deflated_sharpe.py` (heuristique) retiré (X-01). Activable via `optimizer.deflated_sharpe_gate` — **défaut `false`** (code et `config/lifecycle.yaml`) : la correction de test multiple n'est PAS active tant qu'elle n'est pas posée à `true`. Tests : `test_deflated_sharpe_gate.py`. |
