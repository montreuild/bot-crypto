...
|---|---|---|---|
| **ML-10** ✅ | **Réoptimiser les seuils ADX** des YAML (`adx_min`, `adx_threshold`, `adx_thresh`…) | M | **Livré 2026-08-05** : campagne Wilder (`scripts/recalibrate_adx_thresholds.py`), échelle mesurée mean ADX ~27–29 (1h/4h/1d), `optimizer_results` + defaults YAML mis à jour — cf. `research/ml10_adx_recalibration.json`. Exclus (trop lents) : v4/v5/ml_dynamic_threshold (retrain online) |
