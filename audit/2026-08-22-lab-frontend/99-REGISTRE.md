...
0 plausible. **Les 10 sont livrés** — les trois causes et les deux constats
indépendants.
...
| `LAB-04` | **P1** | Presets annonçant 60 essais / ~10 min ; effectif médian 135, max 400, 40/41 stratégies au-dessus | `optimizer/status.ts:6-8` | B | **livré** |
...
| `LAB-05` | P2 | Budget reproportionné, arrêt anticipé et essais en échec tronquent tous le compteur — aucun n'est distingué | `optimizer_search.py:653` | B | **livré** |
| `LAB-07` | P2 | Les 10 recettes et les 14 stratégies n'ont aucun nom commun, affichées l'une sous l'autre sans lien | `ml-view.tsx` | C | **livré** |
| `LAB-08` | P2 | « Prochain retrain » affiche `-154352s` : `timeAgo` calcule un passé | `frontend/src/lib/utils.ts:131` | — | **livré** |
...
| `LAB-06` | P3 | « 1/14 entraînés » (axe stratégie) juxtaposé à 3 entrées de registre (axe tf × recette) | `ml-view.tsx` | C | **livré** |
| `LAB-10` | P3 | `filterMl` transmis à deux enfants sur trois : « Optimiseur ML » liste les espaces des 41 stratégies | `optimizer-view.tsx:55` | — | **livré** |
