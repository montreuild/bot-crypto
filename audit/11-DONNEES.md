...
|---|----------|-------|---------|
| D-01 | 🟠 Majeur | La bougie en formation est persistée dans le cache Parquet | `core/candle_store.py` vs `live/ohlcv_cache.py:128` |
| D-02 | 🟠 Majeur | `unique("time")` sans `keep` : la version conservée n'est pas déterministe | `core/candle_store.py:214,262` |
| D-03 | 🟡 Moyen | Aucune détection de trou à l'écriture, seulement à la lecture par l'API | `api/helpers.py:168-187` |
| D-04 | 🟡 Moyen | 386 Mo de features en cache sans politique d'éviction ni versionnage | `core/feature_store.py` |
| D-05 | 🟡 Moyen | `oos_tracker.json` réécrit intégralement par slot | `core/oos_tracker.py` |
| D-06 | 🟡 Moyen | Les résultats persistés ne portent pas la version du code qui les a produits | `core/backtest_history.py`, `oos_tracker.py` |
| D-07 | 🔵 Mineur | Aucune donnée fraîche depuis le 2026-08-07 | `data/` |
