...
# ════════════════════════════════════════════════════════════════════════════
#  MLStrategyTrainer — réentraînement périodique des stratégies BaseStrategyML
# ════════════════════════════════════════════════════════════════════════════
class MLStrategyTrainer:
    """
    Gère le cycle de vie des modèles ML en production :
      - Chargement des modèles persistés au démarrage du LiveTrader.
      - Réentraînement périodique en arrière-plan (thread daemon).
      - Persistance automatique après chaque fit réussi.

    Séparation des responsabilités :
      LiveTrader  → signaux, ordres, positions (ne sait pas entraîner des modèles)
      MLStrategyTrainer → fetch de données, fit, save/load (ne sait pas trader)

    Usage dans LiveTrader.__init__ :
    ...
    def __init__(self, cfg: dict):
        self.cfg = cfg
        self._retrain_at: Dict[str, float] = {}   # name → prochain timestamp d'entraînement

    # ── Démarrage ────────────────────────────────────────────────────────────
    def load_models(self, strategies: dict, default_tf: str) -> None:
        """
        Parcourt les stratégies, charge les modèles persistés depuis le disque
        ...
        for name, strat in strategies.items():
            ...
            tf   = strat.timeframes[0] if getattr(strat, "timeframes", None) else default_tf
            path = os.path.join(strat.model_dir, f"{name}_{tf}.pkl")
            if strat.load_model(path):
                logger.info(f"[MLTrainer] {name} : modèle chargé depuis {path}")
            else:
                logger.info(f"[MLTrainer] {name} : pas de modèle persisté — sera entraîné au 1er cycle")

    # ── Boucle principale ─────────────────────────────────────────────────
    def maybe_retrain(self, strategies: dict, scanner, default_tf: str) -> None:
        """
        À appeler à chaque cycle de la boucle principale.
        Lance un thread de réentraînement pour chaque stratégie ML dont
        l'intervalle est écoulé.
        Fréquence configurable via strategy_params.<name>.retrain_interval_h
        (défaut : strat.retrain_interval_h, lui-même défaut 6h).
        """
        from app.engine.engine import BaseStrategyML
        now         = time.time()
    ...
    # ── Thread de réentraînement ──────────────────────────────────────────
    def _retrain_thread(self, name: str, strat, strat_params: dict,
                        tf: str, scanner) -> None:
        """Fetch OHLCV → fit → save_model (exécuté dans un thread daemon)."""
        import os
        logger.info(f"[MLTrainer] Début réentraînement {name}/{tf}…")
        try:
            symbols = scanner.get_symbols()
            # Préférer BTC comme paire de référence (plus liquide, historique riche)
            ...
            if df is None or len(df) < strat.min_bars_required(params):
                logger.warning(f"[MLTrainer] {name} : données insuffisantes pour {symbol}/{tf}")
                return
            strat.fit(df, params)
            model_path = os.path.join(strat.model_dir, f"{name}_{tf}.pkl")
            strat.save_model(model_path)
            logger.info(f"[MLTrainer] {name} réentraîné et sauvegardé → {model_path}")
        except Exception as e:
