...
def _build_features(raw_df: pl.DataFrame) -> Optional[pl.DataFrame]:
    """Construit ~462 colonnes V4 (avec lags 1/3/6/12). Identique à V11 pour
    bénéficier du cache partagé feature_store (name="v4_polars", version="1")."""
...
class Strategy(BaseStrategyML):
    ...
    def _train_impl(self, df: pl.DataFrame, tf_key: str, params: dict) -> bool:
        ...
        if len(np.unique(y_amp[:split])) < 2 or len(np.unique(y_dir[:split])) < 2:
            logger.warning(f"[OmnibusV11-FollowSetup] {tf_key} : labels mono-classe, fit ignoré")
            return False
