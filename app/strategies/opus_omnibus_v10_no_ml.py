"""Opus Omnibus V10 — variante *sans ML* (``opus_omnibus_v10_no_ml``).

Jumeau déterministe et **autonome** de ``opus_omnibus_v10`` (V9 corrigé :
LONG_TU strict, SIGNAL_UP à risque dynamique, LONG_RANGE_LIGHT). Même routing
(régime simple ADX+alignement SMA, 8 setups, fenêtre exit_td, excès baissier)
mais ``p_event``/``p_up`` sont des proxys d'indicateurs au lieu des modèles V4.
...
class Strategy(BaseStrategy):
    """V10 OMNIBUS sans ML — LONG_TU strict, SIGNAL_UP dynamique, proxys d'indicateurs."""
    ...
    def score(self, df: pl.DataFrame, params: dict = None,
        ...
        sig["indicators"] = {
            ...
            "bearish_excess": bool(bearish_excess), "signal_up_dyn": bool(signal_up_dyn_applied),
            "p_event": round(p_event, 4), "p_up": round(p_up, 4),
        ...
        if setup["name"] == "SIGNAL_UP":
            conditions.append(
                f"Excès baissier : rouge×2={consec_red} | "
                f"RSI<{p.get('bearish_excess_rsi_threshold', 38):.0f}={rsi_excess} | "
                f"Prix<SMA20-{p.get('bearish_excess_sma_pct', 1.5):.1f}%={price_below_sma20}")
            if signal_up_dyn_applied:
                conditions.append(
                    f"SIGNAL_UP dynamique : size×{size_factor:.2f} SL {sl_atr_mult:.2f}×ATR (régime {regime_lbl})")
        if setup["name"] == "LONG_TU":
            conditions.append(f"LONG_TU strict : ADX={adx_v:.1f} ≥ {setup.get('needs_adx_above', 25):.0f} ✓")
    ...
    def check_early_exit(self, df: pl.DataFrame, position: dict,
        ...
        try:
            ...
            logger.warning(f"[OmnibusV10-NoML] check_early_exit KO : {e}")
            return None
