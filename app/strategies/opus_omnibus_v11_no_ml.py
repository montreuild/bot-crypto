"""Stratégie Opus Omnibus V11 — variante *sans ML* (``opus_omnibus_v11_no_ml``).

Équivalent à base d'indicateurs de :mod:`app.strategies.opus_omnibus_v11`. On
conserve l'intégralité du routing V11 (régime **enrichi** à 4 classes avec
requalification DI/pente, setups V10, SIGNAL_UP dynamique, fenêtre exit_td,
excès baissier, sorties anticipées) ; seules les deux probabilités ML
``p_event`` (amplitude) et ``p_up`` (direction) — produites en V11 par des
modèles LightGBM entraînés inline — sont remplacées par les proxys déterministes
de :mod:`app.strategies._no_ml_proxy`.

On supprime donc tout l'appareillage ML coûteux de V11 : entraînement multi-
horizon, calibration isotone, importance/élagage des features, persistance de
modèles. Le pipeline de features V4 (polars) et le cache feature_store
(catalogue ``v4_polars`` v1) restent partagés avec V11/V11-FollowSetup.
"""
...
class Strategy(BaseStrategy):
    """V11 OMNIBUS sans ML — régime enrichi + proxys d'indicateurs."""
    ...
    def prepare_for_backtest(self, df: pl.DataFrame) -> None:
        try:
            ...
            logger.info(
                f"[OmnibusV11-NoML] backtest : features pré-calculées sur "
                f"{self._bt_features_len} bougies "
                f"({(len(feats.columns) if feats is not None else 0)} colonnes)"
            )
        except Exception as e:
            logger.warning(f"[OmnibusV11-NoML] prepare_for_backtest KO : {e}")
    ...
    def score(self, df: pl.DataFrame, params: dict = None,
        ...
        sig["reason"] = (
            f"OmnibusV11-NoML {setup['name']} {side.upper()} | {regime_lbl}/{regime_sub} | tf={tf} | "
    ...
    def check_early_exit(self, df: pl.DataFrame, position: dict,
        ...
        try:
            ...
            logger.warning(f"[OmnibusV11-NoML] check_early_exit recompute KO : {e}")
            return None
