...
def test_update_requires_both_tf_and_symbol(fake_cfg):
    with pytest.raises(HTTPException) as e:
        update_strategy_params(
            None,
            StrategyParamsBody(
                strategy="trend_rider", params={"adx_min": 20}, timeframe="4h",
            ),
        )
...
def test_override_write_goes_through_apply_best_params(fake_cfg, monkeypatch):
    ...
    res = update_strategy_params(
        None,
        StrategyParamsBody(
            strategy="trend_rider",
            params={"chop_max": 58.0},
            timeframe="4h",
            symbol="ETH/USDC",
        ),
    )
...
def test_base_write_unchanged_without_tf_symbol(fake_cfg, monkeypatch):
    ...
    res = update_strategy_params(
        None,
        StrategyParamsBody(strategy="trend_rider", params={"adx_min": 25}),
    )
