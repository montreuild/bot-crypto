...
class Backtester:
    def __init__(self, engine: Engine, cfg: dict):
        ...
        # Stop initial : distance du stop d'entrée
        self.atr_stop_mult  = float(bcfg.get("atr_stop_mult",  2.0))
        # Trailing : mult de base (s'active après grace_bars)
        self.atr_trail_mult = float(bcfg.get("atr_trail_mult", 1.8))
        # Take profit : multiple de l'ATR depuis l'entrée
        self.atr_tp_mult    = bcfg.get("atr_tp_mult", 3.5)
        # Période de grâce : N barres avant d'activer le trailing
        self.grace_bars     = int(bcfg.get("grace_bars", 3))
        # Breakeven : activer quand profit >= breakeven_r × stop_dist
        self.breakeven_r    = float(bcfg.get("breakeven_r", 0.8))
        # Trailing resserré quand gros profit
        self.trail_tight    = bcfg.get("trail_tight_mult", 1.2)

        self.trailing      = TrailingStopManager(
            mult             = self.atr_trail_mult,
        ...
        # Plafond de notionnel : fraction du capital (augmenté pour laisser le sizing vivre)
    ...
    def run(self, df: pd.DataFrame, symbol: str = "BTC/USDC") -> "BacktestResult":
        ...
        for i in range(warmup, len(df) - 1):
            ...
            if position is not None:
                ...
                # ── Vérification stop intrabar ────────────────────────────────
                # Le stop est vérifié sur le LOW (long) ou HIGH (short) de la barre courante.
                # Prix d'exécution = stop (marché), avec slippage = spread/2 défavorable.
                stop_hit = (side == "long"  and c_low  <= stop) or \
                           (side == "short" and c_high >= stop)

                # ── Vérification take profit intrabar ─────────────────────────
                tp_hit = tp is not None and (
                    (side == "long"  and c_high >= tp) or
                    (side == "short" and c_low  <= tp)
                )

                if stop_hit or tp_hit:
                    if tp_hit and not stop_hit:
                        # TP touché en premier → prix favorable
                        exec_price = tp
                        exit_reason = "take_profit"
                    else:
                        # Stop hit → prix défavorable (slippage réaliste)
                    ...
                    # ── Mise à jour du trailing stop avec grace_bars + breakeven ──
                    atr      = _atr(window)
                    c_close  = float(current_bar["close"])
                    bars_held_now = i - position["bar"]
                    new_stop = self.trailing.update_stop(
                ...
                continue  # On a traité la position → barre suivante
            ...
            # ── Stop initial : priorité au stop proposé par la stratégie ─────
            # Certaines stratégies (pullback, breakout) ont des stops naturels
            # différents du stop ATR générique. Le signal peut proposer un mult.
            strategy_stop_mult = signal.get("stop_atr_mult", self.atr_stop_mult)
            stop = (exec_price - atr * strategy_stop_mult if signal["side"] == "long"
                    else exec_price + atr * strategy_stop_mult)

            # ── Take profit optionnel ──────────────────────────────────────────
            tp = None
            if self.atr_tp_mult:
                tp = (exec_price + atr * self.atr_tp_mult if signal["side"] == "long"
                      else exec_price - atr * self.atr_tp_mult)
            ...
            # Plafond position : configurable (défaut 50% du capital)
